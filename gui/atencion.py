...
class AtencionWindow(QMainWindow):
    def __init__(self):
        ...
        # Validación del campo "No. Cliente"
        self.reg_orden.setValidator(QIntValidator(0, 999999, self))
        self.reg_orden.setMaxLength(6)

        # Validación del campo "Buscar" en actualizar y eliminar
        self.act_buscar.setValidator(QIntValidator(0, 999999, self))
        self.act_buscar.setMaxLength(6)
        ...
        self.act_orden.setDisabled(True)
        self.act_cliente.setDisabled(True)
        self.act_motivo.setDisabled(True)
        self.act_fecha.setDisabled(True)
        self.act_estatus.setDisabled(True)
    ...
    def modificar_llamadas(self):
        if self.llamada != '':
            ...
            if estado_nuevo == "--- Seleccione una opción":
                QMessageBox.warning(self, "Advertencia", "Debe seleccionar un estado válido.")
                return
            act = self.base_datos.actualiza_llamadas(self.Id, self.act_orden.text().upper(), self.act_cliente.text().upper(),
                                                     self.act_motivo.text().upper(), self.act_fecha.text().upper(),
                                                     self.act_estatus.text().upper(), estado_nuevo)
            if act == 1:
...
if __name__ == '__main__':
    ...
    ventana_atencion = AtencionWindow()
